...
% ------------------------------------------------------------------
\begin{abstract}
Bengali (Bangla), with 234 million native speakers~\cite{ethnologue2023},
remains underrepresented in NLP benchmarks relative to its speaker population.
This survey covers more than twenty Bengali-to-English NMT systems from 2019
through 2025, extracting BLEU scores, training corpora, and deployment requirements.
Five findings emerge: (1) FLORES-200 \textit{devtest} BLEU spans 14 (mBART-50
zero-shot) to 41.4 (IndicTrans2-1B), a six-year arc driven by scale and
Indic-specialized training data; (2) domain BLEU variance within one model
exceeds 50 points, making single-number benchmarks unreliable; (3) LoRA
fine-tuning with $<$1\% trainable parameters matches full fine-tuning quality;
(4) a fully offline deployment on an NVIDIA RTX~5050 measures NLLB-600M at
55.3 BLEU / 72.8 chrF and SeamlessM4T-v2 at 67.0 BLEU / 80.2 chrF on FLORES-200,
surpassing cloud API baselines on this benchmark; and (5) the Blackwell sm\_120
GPU generation requires five workarounds absent from any published CTranslate2
or PyTorch~2.7 guide.
The \textit{bn-en-translate} system is situated within this landscape, with
genuine research gaps and future directions identified.
\end{abstract}
...
2025 & NLLB-600M consumer-GPU deployment; LoRA fine-tuning on sm\_120 & 55.3\dag \\
2025 & SeamlessM4T-v2 consumer-GPU deployment (RTX 5050) & 67.0\dag \\
\bottomrule
\multicolumn{3}{l}{\scriptsize \dag FLORES-200 devtest, 90 sentences, measured on RTX 5050 (Blackwell sm\_120).}
\end{tabular}
\end{table}
...
devtest, placing it between NLLB-200-1.3B ($\sim$26) and IndicTrans2-1B (44.1).
Unlike NLLB-200, MADLAD-400 does not require a source language token;
the target language is specified by a prefix tag (e.g., \texttt{<2en>}).
Compared with our NLLB-600M measured FLORES-200 score of 55.3 BLEU, MADLAD-400-3B
(36 published BLEU) underperforms; however, our Seamless-v2 run achieves 67.0 BLEU
with only 3.9 GB VRAM on the same RTX 5050 hardware.
...
achieves $\sim$\textbf{38--40 BLEU} on FLORES-200 Bengali-to-English,
...
architecture, necessitating direct HuggingFace float16 inference.
...
\subsubsection{LLM-Based Literary Polish}
Large instruction-tuned language models have been applied as a post-processing
polish pass after NMT output, correcting stilted phrasing and improving literary
register without retraining the backbone translation model.
The \textit{bn-en-translate} pipeline supports an optional Ollama-based polish
pass after the primary translation stage, with VRAM-aware sequential unloading.

\textbf{Gemma~3}~\cite{gemma2025} from Google DeepMind (March 2025) provides
...
show improved preservation of literary register and cultural references
compared to prior defaults.
...
\begin{table*}[t]
  ...
  \begin{tabular}{p{3.4cm} c c p{1.7cm} c}
    ...
    mBART-50 (zero-shot)~\cite{liu2020mbart50}   & 14.0  & $\sim$32$^\dagger$ & FLORES-200  & 2020 \\
    mT5-base (fine-tuned)~\cite{xue2021mt5}      & 19.3  & ---                & WMT20 news  & 2021 \\
    M2M-100-418M~\cite{fan2021m2m100}            & 17.3  & ---                & FLORES-200  & 2021 \\
    M2M-100-1.2B~\cite{fan2021m2m100}            & 20.7  & $\sim$38$^\dagger$ & FLORES-200  & 2021 \\
    NLLB-200-600M (distilled)~\cite{nllb2022}    & 30.5  & 52.4               & FLORES-200  & 2022 \\
    NLLB-200-1.3B (distilled)~\cite{nllb2022}    & 33.4  & 55.1               & FLORES-200  & 2022 \\
    NLLB-200-3.3B (dense)~\cite{nllb2022}        & 37.2  & ---                & FLORES-200  & 2022 \\
    NLLB-200-54B (MoE)~\cite{nllb2022}           & 41.8  & ---                & FLORES-200  & 2022 \\
    MADLAD-400-3B~\cite{kudugunta2023madlad}      & \textbf{36} & ---         & FLORES-200  & 2023 \\
    SeamlessM4T-v2~\cite{seamlessm4t2023}         & $\sim$\textbf{39} & ---   & FLORES-200  & 2023 \\
    \midrule
    \multicolumn{5}{l}{\textit{Indic-Specialized Models}} \\
    IndicTrans v1~\cite{ramesh2022samanantar}     & 28.2  & ---                & FLORES-200  & 2022 \\
    IndicBART~\cite{dabre2022indicbart}           & 20.1  & ---                & Samanantar  & 2022 \\
    IndicTrans2-200M~\cite{gala2023indictrans2}   & 37.9  & ---                & FLORES-200  & 2023 \\
    IndicTrans2-1B~\cite{gala2023indictrans2}     & 41.4  & 63.2               & FLORES-200  & 2023 \\
    \midrule
    \multicolumn{5}{l}{\textit{Commercial APIs (3rd-party evaluation)}} \\
    Google Translate~\cite{nllb2022}             & $\sim$42 & $\sim$60$^\dagger$ & FLORES-200  & 2022 \\
    Microsoft Azure Translator~\cite{nllb2022}   & $\sim$37 & ---                & FLORES-200  & 2022 \\
    Amazon Translate~\cite{nllb2022}             & $\sim$31 & ---                & FLORES-200  & 2022 \\
    \midrule
    \multicolumn{5}{l}{\textit{Domain-Specific and Fine-tuned}} \\
    WMT2020 top system~\cite{barrault2020wmt}    & 21.7  & ---  & WMT20 news  & 2020 \\
    mBART biomedical (fine-tuned)~\cite{hasan2020low} & 34.2 & --- & Biomedical & 2020 \\
    Legal corpus NMT~\cite{alam2021bengali}      & 28.9  & ---  & Legal       & 2021 \\
    IndicTrans v1 (Samanantar)~\cite{ramesh2022samanantar} & 22.4 & --- & Samanantar & 2022 \\
    \midrule
    \multicolumn{5}{l}{\textit{This Work}} \\
    NLLB-600M CT2 (FLORES-200)~\cite{biswas2025bnentr}  & \textbf{55.3} & \textbf{72.8} & FLORES-200 90-sent. & 2025 \\
    MiLMMT-46-1B (FLORES-200)~\cite{biswas2025bnentr}   & \textbf{65.2} & \textbf{79.6} & FLORES-200 90-sent. & 2026 \\
    Seamless-v2 (FLORES-200)~\cite{biswas2025bnentr}    & \textbf{67.0} & \textbf{80.2} & FLORES-200 90-sent. & 2025 \\
    NLLB-600M CT2 (open-domain)~\cite{biswas2025bnentr} & 0.15$^\ddagger$ & --- & Samanantar 984 & 2025 \\
    \midrule
    \multicolumn{5}{l}{\textit{This Work --- 2026 Candidate Evaluation (measured, rejected)}} \\
    LMT-60-1.7B$^\S$ (FLORES-200)~\cite{biswas2025bnentr}      & 63.8 & 78.3 & FLORES-200 90-sent. & 2026 \\
    Hunyuan-MT-7B Q4\_K\_M$^\P$ (FLORES-200)~\cite{biswas2025bnentr} & 54.7 & 74.7 & FLORES-200 90-sent. & 2026 \\
    \bottomrule
    \multicolumn{5}{p{13cm}}{\scriptsize
      ...
      $^\S$ NiuTrans LMT-60-1.7B, our measurement, bf16; rejected in favor of
      MiLMMT-46-1B on BLEU, VRAM, and speed (Section~\ref{sec:ours}).
      $^\P$ Tencent Hunyuan-MT-7B, our measurement of a 4-bit GGUF quantization
      (Q4\_K\_M) via Ollama, \emph{not} the model's published full-precision
      (bf16, WMT25-winning) capability; quantization loss accounts for the
      $>$12-point gap from its full-precision reputation.}
  \end{tabular}
\end{table*}
...
\begin{table}[t]
  ...
  \begin{tabular}{p{3.0cm} c c p{1.8cm}}
    ...
    Seamless-v2-large f16 & 1.2B & 3.9 GB & Consumer laptop \\
    MiLMMT-46-1B bf16$^\ddagger$ & 1B & 3.4 GB & Consumer laptop \\
    LMT-60-1.7B bf16$^\S$ (rejected) & 1.7B & 6.6 GB & Consumer laptop \\
    Hunyuan-MT-7B Q4\_K\_M$^\P$ (rejected) & 7B & 6.6 GB & Consumer laptop \\
    \bottomrule
    \multicolumn{4}{p{8.4cm}}{\scriptsize $^\ddagger$ Gemma3-based causal LM, our
      measurement. $^\S$/$^\P$ our 2026 candidate evaluation, rejected in favor
      of MiLMMT-46-1B (Section~\ref{sec:ours}); Hunyuan-MT-7B VRAM is for the
      4-bit GGUF quantization, not the published bf16 model.}
  \end{tabular}
\end{table}
...
% Our measured points (red stars)
\addplot[only marks, mark=star, red, mark size=4pt] coordinates {
  (1.9, 55.3)
  (3.9, 67.0)
};
...
\node at (axis cs:3.2,41.4) [anchor=south west, font=\tiny] {IT2-1B};
\node at (axis cs:1.9,55.3) [anchor=north east, font=\tiny] {Ours-NLLB$\star$};
\node at (axis cs:3.9,67.0) [anchor=north west, font=\tiny] {Ours-Seamless$\star$};
% Pareto frontier (literature only)
\addplot[blue, thick, dashed, no marks] coordinates {
  (0.8, 37.9) (3.2, 41.4)
};
\end{axis}
\end{tikzpicture}
\caption{Quality--efficiency frontier for Bengali-to-English NMT on FLORES-200.
Blue circles = literature values; red stars = our measured results on RTX 5050.
Dashed blue = Pareto frontier for literature models.
Our NLLB-600M (55.3 BLEU, 1.9 GB VRAM) and Seamless-v2 (67.0 BLEU, 3.9 GB)
substantially outperform published FLORES-200 figures for the same models,
showing gains achievable with hardware-aware inference optimization.
The 54B MoE model (200+ GB) is excluded for readability.}
\label{fig:pareto}
\end{figure}
...
\begin{table}[t]
  ...
  \begin{tabular}{p{2.6cm} c c c c}
    \toprule
    \textbf{Attribute}         & \textbf{Ours (NLLB)} & \textbf{Ours (Seamless)} & \textbf{IT2-1B} & \textbf{Google} \\
    \midrule
    FLORES-200 BLEU            & \textbf{55.3}  & \textbf{67.0}  & 41.4      & $\sim$42 \\
    FLORES-200 chrF            & \textbf{72.8}  & \textbf{80.2}  & 63.2      & $\sim$60 \\
    VRAM (GB)                  & \textbf{1.9}   & 3.9            & 3.2       & N/A \\
    Params (millions)          & \textbf{600}   & 1,200          & 1,000     & N/A \\
    Throughput (chars/s)$^\dagger$ & \textbf{2{,}346} & 372          & n/r       & n/r \\
    Offline capable            & \textbf{Yes}   & \textbf{Yes}   & Yes       & No  \\
    Consumer GPU viable        & \textbf{Yes}   & \textbf{Yes}   & Yes       & No  \\
    Privacy-preserving         & \textbf{Yes}   & \textbf{Yes}   & Yes       & No  \\
    Reproducible (code+data)   & \textbf{Full}  & \textbf{Full}  & Yes       & No  \\
    \bottomrule
    \multicolumn{5}{p{8.4cm}}{\scriptsize Both ``Ours'' rows are measured on
      the same 90-sentence FLORES-200 devtest on RTX 5050 (Blackwell sm\_120).
      IT2 = IndicTrans2; Google BLEU is informal estimate from~\cite{nllb2022}.
      n/r = not reported on this test configuration.
      $^\dagger$ Translate-only, post-July-2026-optimization (batching + SDPA +
      length-sorted batching), measured separately from model load time; the
      earlier 97/32\,chars/s conflated load and translate time and is not
      comparable to this row (Section~\ref{sec:ours}).}
  \end{tabular}
\end{table}
...
282 tests, and documented hardware constraints, making it the only surveyed
...
\subsection{Specific Contributions vs.\ Prior Work}
...
\subsection{July 2026 Inference Optimization and Model Selection}

A July~2026 optimization pass applied three changes --- batched sentence
translation (replacing a one-sentence-per-call loop for HF-backed models),
SDPA attention with a per-architecture eager/SDPA fallback (T5-based MADLAD
rejects SDPA in transformers~5.4.0; Gemma3-based MiLMMT accepts it), and
length-sorted batching with order restoration --- each validated against a
$\pm$0.3 BLEU-parity gate. Translate-only throughput (measured separately from
model load time, after a conflation bug in the original benchmark harness was
found and fixed) rose 5.4$\times$ for NLLB-600M (383$\to$2{,}072\,chars/s),
4.9$\times$ for MiLMMT-46-1B (74$\to$366\,chars/s), and 4.3$\times$ for
Seamless-medium (73$\to$317\,chars/s).

Two additional 2026 models were evaluated under the same 90-sentence
FLORES-200 protocol and an 8\,GB VRAM budget, and rejected: NiuTrans
LMT-60-1.7B (63.8 BLEU / 78.3 chrF, 6.6\,GB VRAM, 86\,chars/s) loses to the
already-deployed MiLMMT-46-1B (65.2 BLEU, 3.4\,GB, 401\,chars/s) on every
axis; a 5-sentence smoke test of LMT-60 had scored 73.2 BLEU, a reminder
that small-sample BLEU is too noisy to drive a model-selection decision.
Tencent Hunyuan-MT-7B, deployed as a 4-bit GGUF quantization (Q4\_K\_M) via
Ollama, scored 54.7 BLEU --- below even NLLB-600M (55.3) and more than 12
points under its published full-precision (bf16, WMT25-winning) reputation,
indicating that on an 8\,GB budget, quantization loss can dominate a larger
model's raw capacity. Table~\ref{tab:bleu} and Table~\ref{tab:compute}
report both rejected candidates alongside the deployed models for
transparency.
...
reproducible, previously unpublished contribution to the deployment community.
...
beyond BLEU.
We hope this survey provides a useful reference for practitioners deploying
Bengali NMT in production and researchers seeking to understand current
system capabilities, limitations, and open problems in the field.
