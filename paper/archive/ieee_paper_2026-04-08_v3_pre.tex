...
\newcommand{\INDICBLEUTWO}{44}
% Project benchmark results — fill in after running scripts/benchmark.py --models all
% Commands defined but not yet used in text; models require separate download.
% Run: python scripts/benchmark.py --models madlad-400-3B seamlessm4t-v2 --sentences 90
\newcommand{\MADLADPROJBLEU}{(benchmark pending model download)}
\newcommand{\SEAMLESSPROJBLEU}{(benchmark pending model download)}
...
\begin{abstract}
% ============================================================
We describe \textbf{bn-en-translate}, a fully offline Bengali-to-English neural
machine translation system for consumer-grade GPU hardware.
NLLB-200-distilled-600M~\cite{nllb2022} runs via CTranslate2~\cite{ctranslate2_klein}
float16 on a single NVIDIA RTX~5050 Laptop GPU (Blackwell sm\_120, 8\,GB
GDDR7)~\cite{nvidia_blackwell}, reaching \textbf{65.2 BLEU} on a 90-sentence
in-domain corpus across 10 domains (4.7 News to 80.6 Health) at
97--100\,chars/s with 82--84\% average GPU utilisation.
LoRA fine-tuning~\cite{hu2021lora} via PEFT~\cite{peft2022} with bf16 precision
trains 3~epochs over 7,863 Samanantar pairs in \TRAINDURATION, yielding \POSTFTBLEU{}
BLEU on the 984-pair open-domain test set (sacreBLEU, no SentencePiece normalization).
A real-time resource monitor records per-run CPU, RAM, GPU, and disk metrics to
SQLite with automated regression detection.
Six hardware-specific deployment constraints on first-generation Blackwell hardware
are documented and resolved, including a cuBLAS INT8 incompatibility, a
concurrent-pip-install SIGBUS, a fast-tokenizer fork-deadlock, a
\texttt{prefetch\_factor} misconfiguration, an fp16/bf16 GradScaler conflict,
and a CTranslate2 batch OOM.
...
\begin{enumerate}
  \item A four-stage translation pipeline (Preprocessor, Chunker, CTranslate2 Translator,
        Postprocessor) achieving 65.2 BLEU overall (up to 80.6 on the Health domain)
...
SeamlessM4T-v2~\cite{seamlessm4t2023} & \SEAMLESSBLEU{} & FLORES-200 & HF native \\
...
the RTX~5050. The RTX~5050 Laptop GPU is consumer hardware retailing around \$300;
this result confirms it can sustain a production parameter-efficient fine-tuning
